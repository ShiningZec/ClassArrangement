...
class CourseScheduler:
    ...
    def __init__(self, course_file: str):
        ...
        for course in self.courses.values():
            self.check_course_priority(course)
    ...
    def schedule_courses(self,
        ...
        while semester_idx < 8 and (len(completed) < classinf
    ...
    def check_course_priority(self, course: Course):
        """将先修课的优先级上调"""
        prereqs: List[str] = course.prerequisites
        priority: int = course.priority
        for prereq in prereqs:
            precourse: Course = self.courses[prereq]
            if (precourse.priority > priority):
                precourse.priority = priority
                self.check_course_priority(precourse)
...
if __name__ == '__main__':
    ...
    cs.schedule_courses(output_file="data\\schedule.json",
                        min_credits=200,
                        classinf=100)
