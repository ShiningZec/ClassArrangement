...
class CourseSchedulerApp:
    ...
    def __init__(self):
        ...
        # Fonts
        self.font_cn = ("SimSun", 16)
        self.font_en = ("Times New Roman", 16)
        # Colors
        # self.bg_color = "#081f29"
        self.bg_color = "#292D3E"
        ...
        self.separator = ctk.CTkFrame(self.root,
                                      width=10,
    ...
    def show_page(self, page):
        ...
        if page == "sub1":
            try:
                ...
                self.schedule_table = self.cli.get_schedule_table(
                    self.sem_var.get())
    ...
    def build_main_page(self, parent):
        ...
        btn_init = ctk.CTkButton(col1,
                                 text="读入课程",
                                 font=self.font_cn,
                                 command=self.init_cs)
        btn_init.pack(pady=5)
        self.min_credit_var = ctk.StringVar(value="100")
        entry_credit = ctk.CTkEntry(col1,
                                    placeholder_text="最低学分",
                                    textvariable=self.min_credit_var)
        entry_credit.pack(pady=5)
        btn_run = ctk.CTkButton(col1,
                                text="生成课表",
                                font=self.font_cn,
                                command=self.run_cs)
        btn_run.pack(pady=5)
        btn_show = ctk.CTkButton(col1,
                                 text="展示课表",
                                 font=self.font_cn,
                                 command=self.get_schedule_table)
        btn_show.pack(pady=5)
        btn_check = ctk.CTkButton(col1,
                                  text="检查正确性",
                                  font=self.font_cn,
    ...
    def build_subpage1(self, parent):
        ...
        ctk.CTkLabel(col1, text="课程ID/关键字:",
                     font=self.font_cn).pack(pady=2, anchor="w")
        ...
        ctk.CTkLabel(col1, text="教学班ID:", font=self.font_cn).pack(pady=2,
                                                                  anchor="w")
    ...
    def build_subpage2(self, parent):
        ...
        for j, d in enumerate(days):
            ctk.CTkLabel(grp3, text=d, width=70,
                         font=self.font_cn).grid(row=0, column=j + 1)
        ctk.CTkLabel(grp3, text="行全选", width=70,
                     font=self.font_cn).grid(row=0, column=8)
        # Checkboxes for each time slot
        self.forbid_vars = [[ctk.IntVar(value=0) for _ in range(7)]
                            for _ in range(13)]
        ...
        for i in range(13):
            ctk.CTkLabel(grp3, text=str(i + 1), width=70,
                         font=self.font_cn).grid(row=i + 1, column=0)
            for j in range(7):
                cb = ctk.CTkCheckBox(grp3,
                                     text="",
                                     variable=self.forbid_vars[i][j],
                                     command=self.update_forbid_time)
                cb.grid(row=i + 1, column=j + 1)
            # Row select checkbox
            cb_row = ctk.CTkCheckBox(grp3,
                                     text="",
        ...
        for j in range(7):
            cb_col = ctk.CTkCheckBox(grp3,
                                     text="",
                                     variable=self.col_vars[j],
                                     command=lambda c=j: self.toggle_column(c))
            cb_col.grid(row=14, column=j + 1)
        cb_all = ctk.CTkCheckBox(grp3,
                                 text="",
    ...
    def create_schedule_display(self):
        ...
        self.sem_week_label = ctk.CTkLabel(
            self.sch_frame,
            text=f"{self.sem_var.get()+1}-{self.week_var.get()+1}",
            width=120,
            height=70,
            fg_color=self.bg_color,
            text_color="#ebc06b",
        ...
        for j, day in enumerate(days):
            ...
            lbl = ctk.CTkLabel(self.sch_frame,
                               text=day,
                               width=w,
                               height=70,
                               fg_color=self.bg_color,
        ...
        self.cell_labels = {}
        for i in range(1, 14):
            lbl_time = ctk.CTkLabel(self.sch_frame,
                                    text=str(i),
                                    width=120,
                                    height=70,
                                    fg_color=self.bg_color,
    ...
    def search_courses(self):
        ...
        if len(results) == 0:
            self.output_text.insert("end", "<No Item>\n")
        for course, attended in results.items():
            line1 = (f"{course.id}, {course.name}, "
                     f"{course.required}, {'已选' if attended else '未选'}\n")
            self.output_text.insert("end", line1)
            for off in course.offerings:
                line = (f"    {off.id}, {off.teacher}, "
                        f"{off.times}, {off.weeks}\n")
    ...
    def display_schedule(self):
        ...
        for day in range(7):
            ...
            for slot_idx, sched in enumerate(self.schedule_table[day]):
                ...
                if day < 5:  # Mon-Fri
                    disp = (name[:5] + "..." +
                            name[-3:]) if len(name) > 9 else name
                else:  # Sat-Sun
                ...
                if lbl:
                    ...
                    lbl.bind("<Button-1>",
                             lambda e, s=sched: self.show_schedule_details(s))
    ...
    def show_schedule_details(self, sched):
        ...
        messagebox.showinfo("课程信息", info, parent=self.root)
